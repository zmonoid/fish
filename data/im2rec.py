...
def make_list(args):
    ...
    test_image_list = list_image(args.test_root, args.recursive, args.exts)
    test_image_list = list(test_image_list)
    ...
    N = len(image_list)
    N_ = len(test_image_list)
    chunk_size_ = (N_ + args.chunks - 1) / args.chunks
    chunk_size = (N + args.chunks - 1) / args.chunks
    for i in xrange(args.chunks):
        chunk = image_list[i * chunk_size:(i + 1) * chunk_size]
        chunk_ = test_image_list[i * chunk_size_:(i + 1) * chunk_size_]
        ...
        if args.test_root is not None:
            write_list(args.prefix + str_chunk + '_test.lst', chunk_[:])
...
def parse_args():
    ...
    parser.add_argument('test_root', default=None, help='path to folder containing images.')
    ...
    cgroup.add_argument('--exts', type=list, default=['.jpeg', '.jpg', '.~1~', '.~2~', '.~3~', '.png'],
